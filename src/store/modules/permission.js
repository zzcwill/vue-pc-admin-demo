...
export const getApiRoutes = (apiData) => {
  ...
  apiData.forEach((oneMenu) => {
    let { menuName, menuPicUrl, menuUrl, menuLevel, nextSubs } = oneMenu;
...
export const formatRoutes = (aMenu) => {
  const aRouter = [];
  aMenu.forEach((oMenu) => {
    const { path, component, name, meta, redirect, children } = oMenu;
  ...
  aRouter.push({ path: '*', redirect: '/nopage', hidden: true });
